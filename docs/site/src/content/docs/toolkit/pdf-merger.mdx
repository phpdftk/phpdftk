...
The merger combines two inputs (<a href={`${base}samples/toolkit/pdf-merger/input-cover.pdf`} target="_blank" rel="noopener noreferrer">cover letter</a> and <a href={`${base}samples/toolkit/pdf-merger/input-resume.pdf`} target="_blank" rel="noopener noreferrer">resume</a>) into a single document.
