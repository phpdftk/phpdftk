...
The example below reads the [page-labels showcase PDF](/samples/writer/page-labels.pdf) and writes both the joined and per-page transcriptions to disk.
...
The captured summary lives at [`samples/reader/metadata.json`](/samples/reader/metadata.json).

## Inspecting structure

`getCatalog()` and `getPages()` expose the document's structural skeleton. The example below walks the [outline showcase PDF](/samples/writer/outline.pdf) and emits a JSON description of its catalog and page tree.

<Example source="reader/extract-structure.php" inputPdf="samples/writer/outline.pdf" />

The summary lives at [`samples/reader/structure.json`](/samples/reader/structure.json).
