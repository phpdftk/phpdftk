...
Phase 3 of the [HTML & SVG rendering roadmap](https://github.com/phpdftk/phpdftk/blob/main/docs/plans/html-and-svg.md). Landed: secure XML loader, `SvgDocument` (with `findById` id→element index), `Group`, all five basic shapes, the `transform` attribute (`Value\Transform`), `<path>` with the full SVG 2 §9.3.9 `d`-grammar, SVG 2 §13 presentation / paint accessors (full CSS Color 3 named-colour table), `<text>`/`<tspan>` with per-glyph positioning + CSS Fonts 4 font accessors, and `<defs>`/`<symbol>`/`<use>` with intra-document href resolution (`href` and legacy `xlink:href`, external refs return null per the security posture). Next: `<clipPath>`/`<mask>`/`<image>`, gradients, and CSS-inside-SVG.
