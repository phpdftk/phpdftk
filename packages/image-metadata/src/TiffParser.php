...
final class TiffParser
{
    ...
    public static function parse(string $data): ImageInfo
    {
        $len = strlen($data);
        if ($len < 8) {
            throw new \RuntimeException('Not enough data for TIFF');
        }
        ...
        if ($magic !== 42) {
            throw new \RuntimeException('Not a valid TIFF file (bad magic)');
        }

        // Offset to first IFD
        $ifdOffset = $readUint32($data, 4);
        if ($ifdOffset + 2 > $len) {
            throw new \RuntimeException('IFD offset out of range');
        }
        ...
        for ($i = 0; $i < $numEntries; $i++) {
            ...
            switch ($tag) {
                ...
                case 257: $height = $value;
                    break;          // ImageLength
                case 258: $bitsPerSample = $value;
                    break;   // BitsPerSample
                case 277: $samplesPerPixel = $value;
                    break; // SamplesPerPixel
                case 262: $photometric = $value;
                    break;     // PhotometricInterpretation
