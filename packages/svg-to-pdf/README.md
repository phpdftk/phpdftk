...
- Explicit `mask` `x` / `y` / `width` / `height` attributes (currently uses masked element's bbox); SVG 2 §14.5.4 default extension (`-10%` / `120%`) of the mask region; `mask-type="alpha"` (only luminance supported); per-element bbox for `<path>` referents (would need to walk path commands for extrema, blocking `<path>` from being clipped or masked via objectBoundingBox).
